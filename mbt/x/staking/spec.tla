...
Delegators == {"d0"}
Validators == {v0, v1}
ValidatorPriorities == <<{v1}, {v0}, {v0, v1}>>
...
\* @type: (ADDR, ADDR, Int, Int, Int) => UNDELEGATION;
Und(d, v, ch, ct, b) == [
        delegator |-> d,
        validator |-> v,
        creationHeight |-> ch,
...
\* @type: (ADDR, ADDR, ADDR, Int, Int, Int, Int) => REDELEGATION;
Red(d, vs, vd, ch, ct, ib, sd) == [
        delegator |-> d,
        validatorSrc |-> vs,
        validatorDst |-> vd,
        creationHeight |-> ch,
...
CandidateValidatorSets ==
    ...
    IN {FoldSeq(Combine, {}, ValidatorPriorities)}
...
Undelegate(delegator, validator, amount) == 
    ...
    Succeed ==
        ...
        /\ UNCHANGED validatorQ
        /\ undelegationQ' = MergedUndelegationQ(undelegationQ,Und(
                delegator,
                validator,
                blockHeight,
                blockTime + PARAM_UnbondingTime,
                IssuedTokens
            ))
        /\ UNCHANGED redelegationQ
        /\ UNCHANGED lastValSet 
...
BeginRedelegate(delegator, src, dst, amount) == 
    ...
    Succeed ==
        ...
        /\ UNCHANGED undelegationQ
        /\ redelegationQ' = 
            CASE IsBonded(src) -> MergedRedelegationQ(redelegationQ, Red(
                        delegator,
                        src,
                        dst,
                        blockHeight,
              ...
              [] IsUnbonding(src) -> MergedRedelegationQ(redelegationQ, Red(
                        delegator,
                        src,
                        dst,
                        unbondingHeight[src],
...
P0 ==  ~(8 < steps /\ action.nature = "beginRedelegate" /\ outcome = "fail:insufficient_shares")
P1 ==  ~(8 < steps /\ action.nature = "beginRedelegate" /\ outcome = "fail:has_receiving_redelegation")
P2 ==  ~(8 < steps /\ action.nature = "beginRedelegate" /\ outcome = "fail:redelegate_to_same_validator")
P3 ==  ~(8 < steps /\ action.nature = "beginRedelegate" /\ outcome = "succeed")
P4 ==  ~(8 < steps /\ action.nature = "undelegate" /\ outcome = "fail:insufficient_shares")
P5 ==  ~(8 < steps /\ action.nature = "undelegate" /\ outcome = "succeed")
P6 ==  ~(8 < steps /\ action.nature = "delegate" /\ outcome = "fail:insufficient_tokens")
P7 ==  ~(8 < steps /\ action.nature = "delegate" /\ outcome = "succeed")
P8 == ~(8 < steps /\ action.nature = "endBlock" /\ outcome = "succeed")
...
~~~~~~~~~~~
*)
P9 == ~(8 < steps /\ action.nature = "beginRedelegate" /\ outcome = "fail:insufficient_tokens")
P10 == ~(8 < steps /\ action.nature = "beginRedelegate" /\ outcome = "fail:invalid_ex_rate")
P11 == ~(8 < steps /\ action.nature = "undelegate" /\ outcome = "fail:insufficient_tokens")
P12 == ~(8 < steps /\ action.nature = "delegate" /\ outcome = "fail:invalid_ex_rate")

(*
~~~~~~~~~~~
The following are less trivial handwritten scenarios.
~~~~~~~~~~~
*)
...
P13 == LET it == 
        \* UndelegateFromUnbondingValidator
        /\ STEPS < steps 
        /\ action.nature = "undelegate"
        /\ status[action.validator] = UNBONDING
        /\ outcome = "succeed"
    IN ~it
...
P14 == LET it == 
        ...
        /\ action.nature = "undelegate"
        /\ status[action.validator] = UNBONDING
        /\ outcome = "fail:insufficient_shares"
    IN ~it

P15 == LET it == 
        \* UndelegateFromUnbondedValidator
        /\ STEPS < steps 
        /\ action.nature = "undelegate"
        /\ status[action.validator] = UNBONDED
        /\ outcome = "succeed"
    IN ~it
...
P17 == LET it == 
        \* RedelegateFromUnbondingValidator
        /\ STEPS < steps 
        /\ action.nature = "beginRedelegate"
        /\ status[action.validatorSrc] = UNBONDING
        /\ outcome = "fail:insufficient_shares"
    IN ~it
...
P18 == LET it == 
        ...
        /\ action.nature = "beginRedelegate"
        /\ status[action.validatorSrc] = UNBONDING
        /\ outcome = "fail:has_receiving_redelegation"
    IN ~it
...
P19 == LET it == 
        ...
        /\ action.nature = "beginRedelegate"
        /\ status[action.validatorSrc] = UNBONDING
        /\ outcome = "fail:redelegate_to_same_validator"
    IN ~it
...
P21 == LET it == 
        \* RedelegateFromUnbondedValidator
        /\ STEPS < steps 
        /\ action.nature = "beginRedelegate"
        /\ status[action.validatorSrc] = UNBONDED
        /\ outcome = "fail:insufficient_shares"
    IN ~it
...
P25 == LET it == 
        /\ STEPS < steps 
        /\ action.nature = "undelegate"
        /\ status[action.validator] = BONDED
        /\ outcome = "succeed"
    IN ~it
...
P27 == LET it == 
        /\ STEPS < steps 
        /\ action.nature = "beginRedelegate"
        /\ status[action.validatorSrc] = BONDED
        /\ outcome = "fail:insufficient_shares"
    IN ~it
...
P32 == LET it == 
        ...
        /\ action.nature = "delegate"
        /\ status[action.validator] = BONDED
        /\ outcome = "fail:insufficient_tokens"
    IN ~it
